...
import {
	MatchesService,
	type FinalizeMatchSummary,
} from "../matches/matches.service";
import { PrismaService } from "../prisma/prisma.service";
import {
	TournamentsService,
	type TournamentContextInput,
	type ResolvedTournamentContext,
 ...
 */
const SUPPORTED_PROVIDER_TOURNAMENT_SLUGS = [
	"world-cup-2026",
	"liga-argentina-2026",
] as const;
...
export class SportsDataSyncService {
	// Store the current provider context for use in private sync methods
	private currentProvider: SportsDataProvider | null = null;
	private currentProviderKey: string | null = null;

	constructor(
		private readonly prisma: PrismaService,
		private readonly providerFactory: SportsDataProviderFactory,
		private readonly matchesService: MatchesService,
		private readonly tournamentsService: TournamentsService,
	) {}

	/**
	 * Get the provider instance for a given providerKey and set it as the current context.
	 * This allows private sync methods to access the provider via getCurrentProvider().
	 */
	private getProvider(
		providerKey: string | null | undefined,
	): SportsDataProvider {
		this.currentProviderKey = providerKey ?? null;
		this.currentProvider = this.providerFactory.getProvider(
			(providerKey as string | null | undefined) ?? undefined,
		);
		return this.currentProvider;
	}

	/**
	 * Get the current provider from the context set by getProvider().
	 * Throws if called outside of an operation that has set the context.
	 */
	private getCurrentProvider(): SportsDataProvider {
		if (!this.currentProvider) {
			throw new Error(
				"No provider context set. Call getProvider() before accessing the provider.",
			);
		}
		return this.currentProvider;
	}
	...
	/**
	 * Get the current providerKey from the context.
	 */
	private getCurrentProviderKey(): string {
		return this.currentProviderKey ?? "mock";
	}
	...
	async confirmExternalMatchResult(
		...
		if (stagedResult === null) {
			throw new NotFoundException(
				`External match result ${input.externalMatchResultId} was not found`,
			);
		}

		if (
			stagedResult.state !== EXTERNAL_MATCH_RESULT_STATES.PENDING_CONFIRMATION
		) {
		...
		if (
			confirmedResult.matchId === null ||
			confirmedResult.confirmedAt === null
		) {
			throw new ConflictException(
				`External match result ${confirmedResult.id} could not be confirmed`,
			);
		}
	...
	async discardExternalMatchResult(
		...
		if (discardedResult.discardedAt === null) {
			throw new ConflictException(
				`External match result ${discardedResult.id} could not be discarded`,
			);
		}
	...
	async listExternalMatchResults(
		...
		const state =
			input.state ?? EXTERNAL_MATCH_RESULT_STATES.PENDING_CONFIRMATION;

		const resolved = await this.tournamentsService.resolveTournamentContext(
			input.tournamentContext ?? {},
		);
	...
	async listExternalMatchMappingDiagnostics(input?: {
		tournamentContext?: TournamentContextInput;
		...
		const tournamentId = resolved.tournament.id;
		const provider = this.getProvider(resolved.tournament.providerKey);
		...
		const matches = await this.prisma.match.findMany({
			...
			select: {
				...
				externalRefs: {
					where: {
						providerKey: this.getCurrentProviderKey(),
					},
					select: {
						externalId: true,
					},
	...
	async importTournament(
		...
		// Get the provider for this tournament based on its providerKey
		const provider = this.getProvider(resolvedTournament.providerKey);
		const providerTournamentKey = this.resolveProviderTournamentKey(
			provider,
			resolvedTournament,
		);
		const syncRun = await this.startSyncRun(
			resolvedTournament.id,
			this.getCurrentProviderKey(),
			SPORTS_DATA_SYNC_TYPES.IMPORT,
		);
		...
		try {
			const teams = await provider.listTeams(providerTournamentKey);
			...
			const venues = await provider.listVenues(providerTournamentKey);
			...
			const fixtures = await provider.listFixtures(providerTournamentKey);
			...
			await this.finishSyncRun(syncRun.id, {
				...
				errorMessage:
					error instanceof Error
						? error.message
						: "Unknown sports-data import error",
			});
	...
	async syncResults(tournamentId?: string): Promise<SportsDataSyncSummary> {
		...
		const syncRun = await this.startSyncRun(
			resolvedTournament.id,
			this.getCurrentProviderKey(),
			SPORTS_DATA_SYNC_TYPES.RESULTS,
		);

		let stagedCount = 0;

		try {
			const results = await provider.listFinalResults(providerTournamentKey);

			for (const result of results) {
				const staged = await this.stageResult(
					resolvedTournament.id,
					syncRun.id,
					result,
				);
			...
			await this.finishSyncRun(syncRun.id, {
				...
				errorMessage:
					error instanceof Error
						? error.message
						: "Unknown sports-data result sync error",
			});
	 ...
	 */
	private assertTournamentSupportsProvider(
		tournament: ResolvedTournamentContext,
	): void {
	...
	private resolveProviderTournamentKey(
		...
		if (
			...
			this.getCurrentProviderKey() === SPORTS_DATA_PROVIDER_KEYS.API_SPORTS ||
			this.getCurrentProviderKey() === SPORTS_DATA_PROVIDER_KEYS.LPF_WEB
		) {
	...
	private async syncVenue(
		...
		const existingReference =
			await this.prisma.externalVenueReference.findFirst({
				where: {
					providerKey: this.getCurrentProviderKey(),
					tournamentId,
	...
	private async syncFixture(
		...
		const homeTeam = await this.resolveTeamIdForTournament(
			tournamentId,
			fixture.homeTeamExternalId,
		);
		const awayTeam = await this.resolveTeamIdForTournament(
			tournamentId,
			fixture.awayTeamExternalId,
		);
		const venueId = await this.resolveVenueId(
			tournamentId,
			fixture.venueExternalId,
		);

		const existingReference =
			await this.prisma.externalMatchReference.findFirst({
				where: {
					providerKey: this.getCurrentProviderKey(),
					tournamentId,
		...
		if (existingReference !== null) {
			...
			if (currentMatch === null) {
				throw new NotFoundException(
					`Match ${existingReference.matchId} was not found`,
				);
			}
	...
	private async resolveTeamId(externalTeamId: string): Promise<string> {
		throw new Error(
			"resolveTeamId now requires tournamentId; use resolveTeamIdForTournament instead",
		);
	}

	private async resolveTeamIdForTournament(
		tournamentId: string,
		...
		if (reference === null) {
			throw new NotFoundException(
				`External team ${externalTeamId} was not found for tournament ${tournamentId}`,
			);
		}
	...
	private async resolveVenueId(
		...
		if (reference === null) {
			throw new NotFoundException(
				`External venue ${externalVenueId} was not found for tournament ${tournamentId}`,
			);
		}
