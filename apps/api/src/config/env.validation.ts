...
function parseOrigins(
	...
	const candidates =
		rawValue.length > 0 ? rawValue.split(",") : [...fallbackOrigins];
...
export function validateEnv(env: Record<string, unknown>): AppEnv {
	...
	return {
		...
		REDIS_HOST:
			typeof env.REDIS_HOST === "string" && env.REDIS_HOST.trim()
				? env.REDIS_HOST.trim()
				: null,
		REDIS_PORT:
			typeof env.REDIS_PORT === "string" && env.REDIS_PORT.trim()
				? requirePort(env.REDIS_PORT, "REDIS_PORT")
				: null,
