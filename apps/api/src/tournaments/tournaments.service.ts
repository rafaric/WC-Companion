...
export class TournamentsService {
	 ...
	 * Resolution order: explicitTournamentId -> selectedSlug -> ACTIVE fallback
	 */
	async resolveTournamentContext(
		input: TournamentContextInput,
	): Promise<ResolvedTournamentContext> {
		// Step 1: Try explicit tournament ID
		if (
			input.explicitTournamentId !== undefined &&
			input.explicitTournamentId !== null &&
			input.explicitTournamentId !== ""
		) {
			...
			if (tournament === null) {
				throw new NotFoundException(
					`Tournament ${input.explicitTournamentId} was not found`,
				);
			}
	...
	private toTournamentMatchView(
		match: ActiveTournamentMatchRecord,
	): TournamentMatchView {
		return {
			id: match.id,
