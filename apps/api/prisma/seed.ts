...
import { SPORTS_DATA_PROVIDER_KEYS } from "../src/sports-data/sports-data.constants";
...
const DEMO_TOURNAMENT = {
	...
	// Demo data uses mock provider
	providerKey: SPORTS_DATA_PROVIDER_KEYS.MOCK,
} as const;
...
const PROVIDER_TOURNAMENT = {
	...
	startsAt: new Date(Date.UTC(2026, 5, 11)),
	endsAt: new Date(Date.UTC(2026, 5, 25)),
	// Real World Cup data via Football-Data.org
	providerKey: SPORTS_DATA_PROVIDER_KEYS.FOOTBALL_DATA,
} as const;
...
const LIGA_ARGENTINA_TOURNAMENT = {
	...
	// LPF data via LPF website/Opta feed
	providerKey: SPORTS_DATA_PROVIDER_KEYS.LPF_WEB,
} as const;
...
async function upsertTournament(
	client: SeedClient,
	tournament:
		| typeof DEMO_TOURNAMENT
		| typeof PROVIDER_TOURNAMENT
		| typeof LIGA_ARGENTINA_TOURNAMENT,
) {
...
async function validateSingleActiveTournament(client: SeedClient) {
	...
	if (activeTournaments.length === 0) {
		throw new Error(
			"Seed validation failed: no ACTIVE tournament exists. One tournament must be ACTIVE.",
		);
	}
	...
	console.log(
		`✓ Validation passed: exactly one ACTIVE tournament (${activeTournaments[0].name})`,
	);
}
...
async function upsertMatch(
	...
	if (!homeTeamId || !awayTeamId) {
		throw new Error(
			`Missing team ids for match ${match.homeTeamName} vs ${match.awayTeamName}`,
		);
	}
...
async function main() {
	await prisma.$transaction(
		async (transaction) => {
			...
			const providerTournament = await upsertTournament(
				transaction,
				PROVIDER_TOURNAMENT,
			);
			console.log(
				`✓ Seeded provider-backed tournament: ${providerTournament.name} (${providerTournament.status})`,
			);

			// Step 2: Seed the Liga Argentina tournament (DRAFT - selectable sync target)
			const ligaTournament = await upsertTournament(
				transaction,
				LIGA_ARGENTINA_TOURNAMENT,
			);
			console.log(
				`✓ Seeded Liga Argentina tournament: ${ligaTournament.name} (${ligaTournament.status})`,
			);

			// Step 3: Seed the demo tournament (FINISHED - for selector testing)
			const demoTournament = await upsertTournament(
				transaction,
				DEMO_TOURNAMENT,
			);
			console.log(
				`✓ Seeded demo tournament: ${demoTournament.name} (${demoTournament.status})`,
			);
			...
			const teamIdsByName = new Map(
				teams.map((team) => [team.name, team.id] as const),
			);
			...
			console.log(
				`✓ Seeded ${teams.length} teams and ${MATCH_SEEDS.length} matches for demo tournament`,
			);
			...
			console.log("\n=== Seed Summary ===");
			console.log(
				`Demo tournament: ${demoTournament.name} (${demoTournament.slug}) - ${demoTournament.status}`,
			);
			console.log(
				`Provider tournament: ${providerTournament.name} (${providerTournament.slug}) - ${providerTournament.status}`,
			);
			console.log(
				`Liga Argentina tournament: ${ligaTournament.name} (${ligaTournament.slug}) - ${ligaTournament.status}`,
			);
			console.log("\nThe selector will show all seeded tournaments.");
			console.log(
				"The ACTIVE tournament serves as the deterministic fallback.",
			);
			console.log(
				"Demo fixtures are preserved under the demo slug for preview.",
			);
		},
