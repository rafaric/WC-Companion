...
export function RecentlyScoredResults({
	items,
	i18n,
}: RecentlyScoredResultsProps) {
	...
	const visibleItems =
		!hasHydrated || !state
			...
			: items.filter(
					(item) =>
						new Date(item.scoredAt).getTime() >
						new Date(state.lastSeenAt).getTime(),
				);

	function clearRecentlyScored() {
		const newestScoredAt =
			visibleItems[0]?.scoredAt ?? new Date().toISOString();
		const nextState = {
			fingerprint,
					...
					>
						<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
							<div className="space-y-1">
								...
								<p className={cn("text-xs", getOutcomeAccentClassName(item))}>
									...
									{formatDateTime(item.scoredAt, i18n.locale)}
								</p>
							</div>
							...
							<div className="grid gap-3 text-sm sm:grid-cols-3 lg:min-w-[520px]">
								<div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
									<p
										className={cn(
											...
											getOutcomeAccentClassName(item),
										)}
									>
										{i18n.final}
									</p>
								...
								<div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
									...
									<p className="mt-1 text-lg font-bold text-white">
										{formatPointsLabel(item.pointsAwarded, i18n)}
									</p>
								</div>
							</div>
						</div>
						...
						<div
							className={cn(
								...
								getExplanationClassName(item.explanationKind),
							)}
						>
