...
function getPredictionOutcomeLabel(
	...
	if (match.prediction.scoringStatus === "SCORED") {
		return formatTemplate(i18n.youEarned, {
			points: formatPointsLabel(match.prediction.pointsAwarded, i18n),
		});
	}
...
function getInitialExpandedMatchId(
	matches: MatchPredictionCard[],
): string | null {
	return (
		matches.find(
			(match) => isOpenForPrediction(match) && match.prediction === null,
		)?.id ?? null
	);
}
...
export function MatchPredictionAccordion({
	...
	const [expandedMatchId, setExpandedMatchId] = useState<string | null>(() =>
		getInitialExpandedMatchId(matches),
	);
	...
	const matchGroups = hasHydrated
		? groupMatchesByDate(matches, i18n.locale)
		: [];
	...
	const safeCurrentGroupIndex = Math.min(
		currentGroupIndex,
							...
							return (
								...
								>
									<button
										...
										className="grid w-full grid-cols-[1fr_auto] items-center gap-3 p-4 text-left transition hover:bg-slate-900 focus-visible:outline-2 focus-visible:outline-cyan-400 sm:p-5"
									>
										<span className="min-w-0">
											...
											<span className="mt-1 block truncate text-xs text-slate-500">
												...
												{formatKickoff(match.kickoffAt, i18n.locale)}
											</span>
										</span>
									...
									{expanded ? (
										<div className="border-t border-slate-800 p-4 pt-5 sm:p-5">
																...
																{scoringExplanation ? (
																	...
																	>
																		...
																		<p className="mt-1 text-sm font-bold">
																			{scoringExplanation.title}
																		</p>
																		<p className="mt-1 text-xs leading-5 opacity-90">
																			{scoringExplanation.detail}
																		</p>
																	</div>
																) : null}
															</div>
											...
											{profileComplete && isMatchOpenForPrediction(match) ? (
												<form
													action={submitPredictionAction.bind(null, match.id)}
													className="mt-0"
												>
													...
													<div className="grid grid-cols-[1fr_auto_1fr] items-end gap-3">
														...
														<span className="pb-5 text-2xl font-black text-slate-600 sm:pb-6">
															–
														</span>
												...
												<p className="mt-5 text-sm leading-6 text-slate-400">
													{matchFinished
														? i18n.matchFinalLocked
														: i18n.matchNoLongerOpen}
												</p>
											) : (
												<p className="mt-5 text-sm leading-6 text-slate-400">
													{i18n.completeProfileToUnlock}
												</p>
											)}
										</div>
									) : null}
