...
export function CopyInviteCodeButton({
	inviteCode,
	showCode = false,
}: CopyInviteCodeButtonProps) {
	const [copied, setCopied] = useState(false);
	const t = useTranslations("groups.common");
	...
	return (
		<button
			...
			aria-label={copied ? t("inviteCodeCopied") : t("copyInviteCode")}
			className={
				showCode
		...
		>
			...
			<span className="sr-only" aria-live="polite">
				{copied ? t("inviteCodeCopiedToClipboard") : ""}
			</span>
		</button>
	);
}
