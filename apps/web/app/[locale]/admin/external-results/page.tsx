...
type AdminExternalResultsTranslator = Awaited<
...
function resolveResultStateFilter(
...
function getListLoadErrorMessage(
	error: unknown,
	t: AdminExternalResultsTranslator,
): string {
	if (!(error instanceof ApiError)) {
...
function getDiagnosticStatusLabel(
	diagnostic: ExternalMatchMappingDiagnosticView,
	t: AdminExternalResultsTranslator,
): string {
...
function getDiagnosticStatusClassName(
	diagnostic: ExternalMatchMappingDiagnosticView,
): string {
	if (!diagnostic.hasExternalReference) {
	...
	if (
		diagnostic.latestExternalResult.state ===
		EXTERNAL_MATCH_RESULT_STATES.CONFIRMED
	) {
...
function getLatestSyncRunByType(
	syncRuns: ExternalSyncRunView[],
	syncType: string,
): ExternalSyncRunView | null {
...
function ResultCard({
	...
	const isPending =
		result.state === EXTERNAL_MATCH_RESULT_STATES.PENDING_CONFIRMATION;
		...
		<article className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/30">
			<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
				<div className="space-y-3">
					...
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
							{t("resultCard.internalMatch")}
						</p>
						{result.match ? (
							<div className="mt-1 space-y-1 text-sm text-slate-200">
								<p className="font-semibold text-white">
									{t("matchup", {
										homeTeam: result.match.homeTeamName,
										awayTeam: result.match.awayTeamName,
									})}
								</p>
								<p>
									{result.match.stage ?? t("resultCard.stageUnavailable")}
									{result.match.groupName ? ` • ${result.match.groupName}` : ""}
								</p>
								<p>
									{result.match.matchId} • {result.match.status} •{" "}
									{formatDateTime(result.match.kickoffAt, locale)}
								</p>
							</div>
						) : (
							<p className="mt-1 text-sm text-amber-200">
								{t("resultCard.notLinked")}
							</p>
						)}
					</div>
				</div>
				...
				<div className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:grid-cols-2 lg:min-w-72">
					...
					<div>
						...
						<p className="mt-1 text-sm font-semibold text-emerald-300">
							{formatStateLabel(result.state, t)}
						</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
							{t("resultCard.playedAt")}
						</p>
						<p className="mt-1 text-sm text-slate-200">
							{formatDateTime(result.playedAt, locale)}
						</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
							{t("resultCard.stagedAt")}
						</p>
						<p className="mt-1 text-sm text-slate-200">
							{formatDateTime(result.stagedAt, locale)}
						</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
							{t("resultCard.confirmedAt")}
						</p>
						<p className="mt-1 text-sm text-slate-200">
							{formatDateTime(result.confirmedAt, locale)}
						</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
							{t("resultCard.discardedAt")}
						</p>
						<p className="mt-1 text-sm text-slate-200">
							{formatDateTime(result.discardedAt, locale)}
						</p>
					</div>
				</div>
			</div>
...
export default async function AdminExternalResultsPage({
	searchParams,
	params,
}: AdminExternalResultsPageProps) {
	const { locale } = await params;
	...
	if (!session) {
		redirect(
			`/auth/login?returnTo=${getLocalizedPath(locale as AppLocale, "/admin/external-results")}`,
		);
	}
	...
	try {
		stagedResults = await getExternalMatchResults(
			accessToken,
			currentState,
			tournamentSlug,
		);
	...
	try {
		matchDiagnostics = await getExternalMatchMappingDiagnostics(
			accessToken,
			tournamentSlug,
		);
	} catch (error) {
	...
	async function confirmExternalResult(formData: FormData) {
		...
		const externalMatchResultId = String(
			formData.get("externalMatchResultId") ?? "",
		).trim();
		const state = resolveResultStateFilter(String(formData.get("state") ?? ""));

		if (!externalMatchResultId) {
			redirect(
				buildExternalResultsPath(locale as AppLocale, state, {
					error: "invalid_input",
				}),
			);
		}
		...
		try {
			...
			redirect(
				buildExternalResultsPath(locale as AppLocale, state, {
					error: getResultErrorCode(error),
				}),
			);
		}

		revalidatePath(
			getLocalizedPath(locale as AppLocale, "/admin/external-results"),
		);
		redirect(
			buildExternalResultsPath(locale as AppLocale, state, {
				success: "confirmed",
			...
			<section className="space-y-6 py-2 sm:py-4">
				...
				<section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/30">
					...
					<div className="mt-5 grid gap-3 lg:grid-cols-2">
						{[
							{
								label: formatSyncTypeLabel("IMPORT", t),
								syncRun: latestImportSyncRun,
							},
							{
								label: formatSyncTypeLabel("RESULTS", t),
								syncRun: latestResultsSyncRun,
							},
						].map(({ label, syncRun }) => (
							<div
								key={label}
								...
								{syncRun ? (
									<div className="space-y-4">
										<div className="flex items-start justify-between gap-3">
											<div>
												<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
													{formatSyncTypeLabel(syncRun.syncType, t)}
												</p>
											...
											<span
												className={`rounded-full border px-3 py-1 text-xs font-semibold ${getSyncRunStatusClassName(syncRun.status)}`}
											>
										...
										<div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
											<div>
												<p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">
													{t("syncRunCard.imported")}
												</p>
												<p className="mt-1 font-bold text-white">
													{syncRun.importedCount}
												</p>
											</div>
											<div>
												<p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">
													{t("syncRunCard.updated")}
												</p>
												<p className="mt-1 font-bold text-white">
											...
											<div>
												<p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">
													{t("syncRunCard.skipped")}
												</p>
												<p className="mt-1 font-bold text-white">
													{syncRun.skippedCount}
												</p>
											</div>
										</div>
										...
										<div className="grid gap-3 text-xs text-slate-400 sm:grid-cols-2">
											<p>
												{t("syncRunCard.started")}:{" "}
												{formatDateTime(syncRun.startedAt, locale)}
											</p>
											<p>
												{t("syncRunCard.completed")}:{" "}
												{formatDateTime(syncRun.completedAt, locale)}
											</p>
										</div>
									...
									<div className="text-sm leading-6 text-slate-300">
										{t("syncRunCard.noSyncYet", { type: label })}
									</div>
								)}
							</div>
						))}
					...
					{syncRuns.length > 2 ? (
						<div className="mt-4 space-y-2">
							<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
								{t("syncRunCard.recentHistory")}
							</p>
							{syncRuns.slice(2).map((syncRun) => (
								<div
									key={syncRun.syncRunId}
									className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-950/40 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"
								>
									...
									<p className="text-xs text-slate-400">
										{formatDateTime(syncRun.startedAt, locale)} ·{" "}
										{t("syncRunCard.imported")} {syncRun.importedCount} ·{" "}
										{t("syncRunCard.updated")} {syncRun.updatedCount} ·{" "}
										{t("syncRunCard.staged")} {syncRun.stagedCount}
									</p>
								</div>
							))}
						</div>
				...
				<details className="group rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/30">
					<summary className="flex cursor-pointer list-none flex-col gap-3 rounded-2xl outline-none transition focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 sm:flex-row sm:items-start sm:justify-between [&::-webkit-details-marker]:hidden">
						<div>
							<p className="text-xs uppercase tracking-[0.2em] text-slate-500">
								{t("diagnosticsSection.badge")}
							</p>
							<h2 className="mt-1 text-lg font-semibold text-white">
								{t("diagnosticsSection.title")}
							</h2>
							<p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
						...
						<div className="flex flex-wrap items-center gap-2 sm:justify-end">
							<p className="rounded-full border border-slate-700 bg-slate-950/60 px-3 py-1 text-xs font-semibold text-slate-300">
								{t("diagnosticsSection.matches", {
									count: matchDiagnostics.length,
								})}
							</p>
									...
									<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
										<div className="min-w-0 space-y-1">
											<p className="text-sm font-semibold text-white">
												{t("matchup", {
													homeTeam: diagnostic.homeTeamName,
													awayTeam: diagnostic.awayTeamName,
												})}
											</p>
											<p className="text-xs text-slate-500">
												{diagnostic.stage ?? t("resultCard.stageUnavailable")}
												{diagnostic.groupName
													? ` · ${diagnostic.groupName}`
													: ""}{" "}
												· {formatDateTime(diagnostic.kickoffAt, locale)}
											</p>
											<p className="break-all text-xs text-slate-500">
												{t("diagnosticCard.internalMatchId")}:{" "}
												{diagnostic.matchId}
											</p>
										</div>
										...
										<div className="grid gap-3 text-sm sm:grid-cols-3 lg:min-w-[520px]">
											<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3">
												<p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">
													{t("diagnosticCard.externalRef")}
												</p>
												<p className="mt-1 break-all font-semibold text-white">
													{diagnostic.externalMatchId ??
														t("diagnosticCard.missing")}
												</p>
											</div>
											<div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3">
												<p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">
													{t("diagnosticCard.latestResult")}
												</p>
												{diagnostic.latestExternalResult ? (
													<p className="mt-1 font-semibold text-white">
														{diagnostic.latestExternalResult.homeScore} -{" "}
														{diagnostic.latestExternalResult.awayScore}
													</p>
												) : (
											...
											<div
												className={`rounded-2xl border p-3 ${getDiagnosticStatusClassName(diagnostic)}`}
											>
												<p className="text-[11px] uppercase tracking-[0.2em] opacity-70">
													{t("diagnosticCard.providerStatus")}
												</p>
												<p className="mt-1 font-semibold">
													{getDiagnosticStatusLabel(diagnostic, t)}
												</p>
											</div>
										</div>
									</div>
				...
				{stagedResults.length > 0 ? (
					...
					<div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 text-sm text-slate-300 shadow-xl shadow-slate-950/30">
						{t("emptyState.noResults", {
							state: formatStateLabel(currentState, t),
						})}
					</div>
				)}
			</section>
